...
Write-Host "正在生成千川直播监测软件..." -ForegroundColor Cyan
& $venvPython -m PyInstaller `
    --noconfirm `
    --clean `
    --onefile `
    --windowed `
    --name "QianchuanLiveMonitor" `
    "qianchuan_monitor_app.py"

if ($LASTEXITCODE -ne 0) {
    throw "软件打包失败，退出码：$LASTEXITCODE"
}

$outputPath = Join-Path $projectRoot "dist\QianchuanLiveMonitor.exe"
Write-Host ""
